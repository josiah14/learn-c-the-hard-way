...
void Database_write(struct Connection* conn) {
  ...
  rc = fflush(conn->file);
  if (rc == -1) {
    die("Could not flush the database.");
  }
}
...
void Database_set(struct Connection* conn,
  ...
  if (addr->set) {
    die("Database address already set.  You must delete it before you can set it, again.");
  }
...
int main(int argc, char* argv[]) {
  if (argc < 3) {
    die("USAGE: ex177 <dbfile> <action> [action params]");
  }
